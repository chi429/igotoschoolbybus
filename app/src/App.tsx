...
import { loadDB, searchStops, type DB, type Place } from './lib/data.ts'
import { findJourneys, fetchEta, type Journey, type EtaInfo } from './lib/journey.ts'
...
export default function App() {
  ...
  const useGps = async () => {
    setGpsBusy(true)
    try {
      // Capacitor plugin: 喺 iOS/Android 行 native 定位，喺瀏覽器自動 fallback 返 navigator.geolocation
  ...
  return (
    <div className="mx-auto min-h-dvh max-w-md pb-16 text-ink">
      <header className="flex items-center justify-between px-4 pb-1 pt-5">
        <h1 className="text-2xl text-hkred">搭咩巴士 🚌</h1>
      ...
      <section className="space-y-3 p-4">
        <LocationField input={fromInput} placeholder="出發地 — 打站名或㩒📍" onGps={useGps} gpsBusy={gpsBusy} />
      ...
      <main className="px-4">
        ...
        {db && (!from || !to) && (
          <p className="py-10 text-center text-sm text-muted">揀好出發地同目的地，即刻話你知搭咩車 ⚡️</p>
        )}
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <div className="flex items-center gap-2">
                      <span
                        className="font-num px-2 py-1.5 text-sm"
                        style={{
                          background: `var(--co-${j.co}-bg)`,
                          color: `var(--co-${j.co}-fg)`,
                        }}
                      >
                        {j.route}
                      </span>
